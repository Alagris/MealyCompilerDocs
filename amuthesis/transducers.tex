...
\begin{center}
	...
	\end{tikzpicture}
\end{center}
The accepting state $q_2\in F$ is marked with double border. Such automaton accepts strings like $bb$,$bbb$,$bab$,$aab$ but rejects $\epsilon$,$a$,$ba$,$aa$ and so on. 
...
\begin{center}
	...
	\end{tikzpicture}
\end{center}
First tape is designated as the input state reading strings over alphabet $\{a,b\}$. The remaining three tapes are the output tapes, printing strings in the respective alphabets $\{0,1\}$, $\{x,y\}$, $\{m,n\}$. In theory there could be any (finite) number of input and output tapes. 
...
This algorithm has been made even more efficient by using techniques from graph theory. Every automaton is a directed graph that could be represented as adjacency matrix\cite{graphTheory}. An example is shown below.
\begin{table}[!htbp]
	\centering
	\begin{tabular}{|l|l|l|l|}
		\hline
...
Let's recall how each function behaves on atomic symbol $\omega$.
...
\begin{center}
	...
	\end{tikzpicture}
\end{center}
In order to use such graph as an automaton it's enough to treat all the outgoing edges state's subsequential output and the input label of every full or incoming edge is directly determined by the target state. Hence this graph becomes the following automaton:
\begin{center}
	\begin{tikzpicture}[scale=0.2]
		\tikzstyle{every node}+=[inner sep=0pt]
		\draw [black] (26.6,-19) circle (3);
...
\begin{center}
	...
	\end{tikzpicture}
\end{center}
Each of those individual vertices could be though of as a separate graph in and of itself but the distinction is blurry. As soon as we add some connection
