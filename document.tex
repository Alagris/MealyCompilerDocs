...
$G : Q \times \Sigma \times T \rightarrow  T \times (\Gamma^* \cup \{\emptyset \})$ is the transition function between memory cells.  $G(q,s,t) =(t',s')$ tells you to copy contents of  cell $t$ at state $q$ , append $s'$ and put resulting string $qs'$  in cell $t'$ of state $\delta(q,s)$. In cases where there exist two conflicting cells like $G(q,s,t_0) =(t',s_0')$ and $G(q,s,t_1) =(t',s_1')$, then you should take $t_0$ if  $t_1s_1' = \emptyset$, or otherwise take $t_1$ if  $t_0s_0' = \emptyset$ is empty. In cases when both $t_0s_0'$ and $t_1s_1'$ are non-empty at the same time, leading to conflict, then automaton immediately halts and rejects.\\
$F : Q \rightarrow (T \cup \{\emptyset\})$ is the accepting function that tells us which memory cell to take the output from. \\
$q_0 : Q \times (T \rightarrow \Gamma^*)$ is the initial state along with some initial assignment to each memory cell. \\
The evaluation of such automata work as follows: \\
$\Sigma = \{a\}, \Gamma=\{0,1\}, T=\{A,B,C,D,E\}$
\begin{center}
	\begin{tikzpicture}[scale=0.2]
	\tikzstyle{every node}+=[inner sep=0pt]
	\draw [black] (13.9,-11.2) circle (3);
	\draw (13.9,-11.2) node {$q_0^{ABC}$};
	\draw [black] (13.9,-21.5) circle (3);
	\draw (13.9,-21.5) node {$q_1^{DE}$};
